...
public class ApiPayController extends ApiBaseAction {
    ...
    public Object payPrepay(@LoginUser UserVo loginUser, Integer orderId) {
        ...
        try {
            ...
            param.put("appid", ResourceUtil.getConfigByName("wx.appId"));
            // 商家账号。
            param.put("mch_id", ResourceUtil.getConfigByName("wx.mchId"));
            String randomStr = CharUtil.getRandomNum(18).toUpperCase();
            // 随机字符串
            param.put("nonce_str", randomStr);
            // 商户订单编号
            param.put("out_trade_no", allOrderId);
            // 商品描述
            param.put("body", body);
            //支付金额
            int amount = allPrice.multiply(new BigDecimal(100)).intValue();
            param.put("total_fee", String.valueOf(amount));
//            System.out.println("***************" + param.get("total_fee") + "***************");
            //System.out.println(orderInfo.getActual_price().multiply(new BigDecimal(100)).intValue() + "***************");
            //param.put("total_fee", 1);
            // 回调地址
            param.put("notify_url", ResourceUtil.getConfigByName("wx.notifyUrl"));
            // 交易类型APP
            param.put("trade_type", ResourceUtil.getConfigByName("wx.tradeType"));
            param.put("spbill_create_ip", getClientIp());
            param.put("openid", loginUser.getWeixin_openid());
            String xml = WXPayUtil.generateSignedXml(param, ResourceUtil.getConfigByName("wx.paySignKey"));
            System.out.println("***************xml=" + xml + "***************");
//            boolean signatureValid = WXPayUtil.isSignatureValid(param, ResourceUtil.getConfigByName("wx.paySignKey"));
//            if (signatureValid) {
//                System.out.println("================微信支付接口签名校验通过================");
//            }
...
//    	}
    ...
    private void sendSms(String order){
        ...
        List<OrderVo> list = orderService.queryByAllOrderId(order);

        //短信发送支付回调
        for(OrderVo orderVo : list){
            //订单号、收货人姓名、手机号、商品信息、地址
            String address = orderVo.getProvince() + orderVo.getCity() +
                    orderVo.getDistrict() + orderVo.getAddress();
            Map<String, Object> params =new HashMap<String, Object>();
            params.put("order_id",  orderVo.getId());
            List<OrderGoodsVo> goodsList = orderGoodsService.queryList(params);
            String goodsName = "";
            for(OrderGoodsVo orderGoodsVo : goodsList){
                goodsName += orderGoodsVo.getGoods_name() + "*" + orderGoodsVo.getNumber() + "/";
            }

            String[] smsParams = {orderVo.getId()+"",orderVo.getConsignee(),orderVo.getMobile(),goodsName,address};
            SmsSingleSenderResult smsResult = smsTXUtils.sendCode("13156218757", "+86",
                    smsParams, 865801);
            JSONObject resultObject = JSONObject.parseObject(String.valueOf(smsResult));
