...
public class ApiUserController extends ApiBaseAction {
	...
	public Object getFUser(@LoginUser UserVo loginUser, int userId) {
		...
		logger.error("=========puserId=============" + userId);
		logger.error("=========userId=============" + loginUser.getUserId());
		if(userId != 0){
			loginUser.setPromoterId(userId);
			UserVo userVo = userService.queryObject( (long)userId );
			userVo.setPromoterName(userVo.getNickname());
			userService.updatefx(loginUser);
		}
